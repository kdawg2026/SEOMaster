...
- **Navigation schema that contradicts robots.txt.** A site added a "Pricing" navbar link pointing at its plan picker, which lives under `/auth/` — a path `robots.txt` disallows. The link is fine for humans, but adding it to the homepage's `SiteNavigationElement` JSON-LD would have advertised to crawlers a URL the same `robots.txt` forbids. The skill now cross-checks every URL in navigation and breadcrumb schema (and the sitemap) against `robots.txt` before it ships, and flags the commercial-page version of the trap: if the only pricing page sits behind a disallowed path it cannot rank for pricing queries at all, so the fix is a crawlable page — better markup cannot rescue it.
- **A date range on a data page must name the endpoint that serves it.** A coverage table advertised `2008-01-02 → today` for a warehouse dataset; a customer pulled the matching COMEX endpoint and got 148 rows starting 2026. Nothing was blocked — the 2008 floor belonged to a different venue (LME) inside a combined table, and the endpoint he called carried the shorter series. The skill now checks that every advertised date range is reachable from a named, documented endpoint (endpoints that exist but appear nowhere on the page are an internal-linking and long-tail gap of their own), and that a range spanning multiple sources carries its caveat in the data model rather than only in the page copy — so an AI sales assistant reading the same rows cannot repeat the overstatement the page just stopped making.
- **A CSP that allows the tag script but not its beacon endpoint.** A Google Ads conversion action was configured on an event the site never sent, and the host the beacon posts to (`googleadservices.com`) was in `script-src` but missing from `connect-src` — so the tag loaded, looked healthy, and any conversion it tried to report would have been blocked by the browser with nothing on the page to explain it. The skill now treats an analytics or conversion host as **two** CSP entries (load it *and* let it send), and verifies that exactly one conversion fires per sale in a real browser against a production build, because a silently blocked beacon and a working one look identical in the tag UI.
